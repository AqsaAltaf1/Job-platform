...
export default function CompanyJobsPage() {
  ...
  const loadCompanyAndJobs = async () => {
    try {
      ...
      // Load company info
      const companyResponse = await fetch(`http://localhost:5000/api/companies/${params.id}`)
      ...
      const jobsResponse = await fetch(`http://localhost:5000/api/jobs?employer_id=${params.id}&page=${currentPage}&limit=12`)
