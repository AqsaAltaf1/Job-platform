...
import ComprehensiveTransparencyDashboard from '@/components/profile/comprehensive-transparency-dashboard';
...
export default function CandidateDashboard() {
  ...
  // Global refresh function that refreshes all dashboard data
  ...
  // Set up polling for live updates (every 20 seconds for better real-time updates)
  useEffect(() => {
    if (user && user.role === 'candidate') {
      ...
      const interval = setInterval(() => {
        ...
        fetchReferences();
        fetchDashboardStats();
      }, 20000); // Reduced to 20 seconds for better real-time updates
  ...
  const fetchSavedJobs = async () => {
    try {
      ...
      showAlert('Failed to load saved jobs', 'error');
    } finally {
  ...
  const fetchVerifiedWorkHistory = async () => {
    try {
      ...
      showAlert('Failed to load work history', 'error');
    } finally {
  ...
  const createVerifiedEmployment = async () => {
    if (!veCompany || !veTitle || !veStartDate) {
      showAlert('Please fill in company, title, and start date', 'error');
      return;
    }
    ...
    try {
      ...
      showAlert('Verified employment record created', 'success');
      ...
      console.error(e);
      showAlert(e.message || 'Failed to create record', 'error');
    } finally {
      setCreatingVE(false);
    }
  };
  ...
  const requestEmploymentReview = async (id: number) => {
    try {
      ...
      if (!workRecord) {
        showAlert('Work record not found', 'error');
        return;
      }

      if (!workRecord.verifier_contact_email) {
        showAlert('Please add verifier email in the edit modal before requesting review', 'error');
        return;
      }
      ...
      const data = await response.json();
      if (response.ok) {
        showAlert('Verification email sent to employer', 'success');
        if (data.data?.review_url) {
          showAlert(`Share this link with your verifier: ${data.data.review_url}`, 'info');
        }
        fetchVerifiedWorkHistory();
      } else if (response.status === 409) {
        showAlert('A review request is already active for this employment. Please wait until it expires.', 'info');
      } else {
        throw new Error(data.error || 'Failed to request review');
      }
    } catch (e:any) {
      console.error(e);
      showAlert(e.message || 'Failed to request review', 'error');
    }
  };
  ...
  const deleteVerifiedEmployment = async (id: number) => {
    try {
      ...
      showAlert('Verified employment removed', 'success')
  ...
  const saveEditVE = async () => {
    ...
    try {
      ...
      showAlert('Verified employment updated', 'success')
      setShowEditVE(false)
      setEditingVE(null)
      fetchVerifiedWorkHistory()
    } catch (e:any) {
  ...
  const fetchReferences = async () => {
    try {
      ...
      showAlert('Failed to load references', 'error');
    } finally {
  ...
  const unsaveJob = async (jobId: string) => {
    try {
      ...
      showAlert('Failed to remove job from saved jobs', 'error');
    }
  };
  ...
  const withdrawApplication = async (applicationId: string) => {
    try {
      ...
      showAlert('Failed to withdraw application', 'error');
    }
  };
  ...
  const toggleReferenceVisibility = async (referenceId: string, isVisible: boolean) => {
    try {
      ...
      if (response.ok) {
        ...
        showAlert(`Reference ${isVisible ? 'made visible' : 'hidden'} successfully`, 'success');
      } else {
        const errorData = await response.json();
        if (errorData.message === 'Cannot toggle visibility for pending references') {
          showAlert('Visibility can only be toggled for completed references', 'info');
        } else {
      ...
      showAlert('Failed to update reference visibility', 'error');
    }
  };
  ...
  const removeReference = async (referenceId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        showAlert('Reference removed successfully', 'success');
      } else {
        const errorData = await response.json();
      ...
      showAlert('Failed to remove reference', 'error');
    }
  };
  ...
  const requestReferenceUpdate = async (referenceId: string) => {
    try {
      ...
      if (response.ok) {
        showAlert('Update request sent to referrer', 'success');
      } else {
        const errorData = await response.json();
      ...
      showAlert('Failed to send update request', 'error');
    }
  };

  const toggleReferenceOutdated = async (referenceId: string, isOutdated: boolean) => {
    try {
      console.log('Toggling reference outdated status:', { referenceId, isOutdated });
      
      const response = await fetch(getApiUrl(`/candidate/references/${referenceId}/outdated`), {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('jwt_token')}`
      ...
      if (response.ok) {
        ...
        showAlert(`Reference marked as ${isOutdated ? 'outdated' : 'current'}`, 'success');
      } else {
        console.error('Toggle failed:', responseData);
        throw new Error(responseData.message || 'Failed to update reference status');
      }
    } catch (error) {
      console.error('Error toggling reference outdated status:', error);
      showAlert(error instanceof Error ? error.message : 'Failed to update reference status', 'error');
    }
  };
  ...
  const sendReferenceRequest = async () => {
    ...
    if (existingReference) {
      showAlert('A reference request has already been sent to this email address', 'error');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        showAlert('Reference request sent successfully', 'success');
      ...
      showAlert(error?.message || 'Failed to send reference request', 'error');
    }
  };
                    ...
                    <div key={reference.id} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex items-center gap-2">
                          {reference.status === 'completed' && (
                            <>
                              ...
                              <div className="flex items-center gap-1">
                                <Switch
                                  checked={!reference.isOutdated}
                                  onCheckedChange={(checked) => toggleReferenceOutdated(reference.id, !checked)}
                                  title="Toggle between current and outdated"
                                />
                          ...
                          {reference.status !== 'completed' && (
                            <span className="text-xs text-muted-foreground">
                              Toggle available after reference is completed
                            </span>
                          )}
                          <div className="flex items-center gap-1">
                            {reference.status === 'completed' ? (
